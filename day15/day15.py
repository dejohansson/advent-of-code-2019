...
def search(steps, x, y, moves, pc):
    ...
    for m in moves:
        ...
        if grid[step(x, y)] == 4:
            status = snapshot.run(m)
            grid[step(x, y)] = status
            if status == 2:
                img.set_data(grid)
                plt.pause(3)
                grid[x, y] = og_val
                s, p = search(steps+1, step(x, y)[0], step(x, y)[1], list(new_moves), snapshot)
                solution = steps+1, (x, y)
            elif status == 1:
                grid[x, y] = og_val
                s, p = search(steps+1, step(x, y)[0], step(x, y)[1], list(new_moves), snapshot)
                if s > 0:
...
def fill(start):
    ...
    while len(next_oxygen) > 0:
        for x, y in list(next_oxygen):
            next_oxygen.remove((x,y))
            grid[x, y] = 2
...
grid = np.full((49, 49), 4, dtype=np.uint8)
...
img = plt.imshow(grid, colors.ListedColormap(['#444444', '#999999', '#aff3ff', 'white', 'black']))
img.set_clim(vmin=0, vmax=4)
xs, ys = 25, 25
grid[xs, ys] = 3
img.set_data(grid)
plt.pause(3)
...
n_steps, oxygen_start = search(0, xs, ys, [1,4,2,3], droid)
print("Steps to reach the oxygen system:", n_steps)
plt.pause(3)
print("Minutes until filled:", fill(oxygen_start))
plt.show()